...
session_start();
include '../db.php';
include 'material_helper.php'; // 👈 helper file included
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_review']) && $isLoggedIn) {
    ...
    if ($rating >= 1 && $rating <= 5 && !empty($reviewText)) {
        ...
        $stmt->bind_param("iiis", $materialId, $userId, $rating, $reviewText);
        $stmt->execute();
        $stmt->close();

        // 2. SELECT Material Title (using new statement)
        $stmt2 = $conn->prepare("SELECT title FROM study_materials WHERE id = ?");
        $stmt2->bind_param("i", $materialId);
        $stmt2->execute();
        $stmt2->bind_result($materialTitle);
        $stmt2->fetch();
        $stmt2->close();

        // 3. Log Activity
        logActivity($conn, $userId, 'review', $materialTitle, "Gave review: $rating stars");
...
// fetch title from DB
$stmt = $conn->prepare("SELECT title FROM study_materials WHERE id = ?");
$stmt->bind_param("i", $materialId);
$stmt->execute();
$stmt->bind_result($materialTitle);
$stmt->fetch();
$stmt->close();

// log activity
logActivity($conn, $userId, 'download', $materialTitle, 'Downloaded Assignment');
...
// Fetch assignments with review and visible_to_user logic
$assignments = [];
if ($selectedSemester && $selectedSubject) {
    $visibleCondition = getVisibleMaterialCondition($userId); // 👈 helper use

    // 👇 Important: Add visible condition in query directly
    $query = "
        ...
        WHERE sm.material_type_id = (SELECT id FROM material_types WHERE type_name = 'Assignment')
...
<head>
  ...
  <title>Assignments - SmartStudy Hub</title>
  ...
  <div class="main-content">
    <div class="container">
      <div class="page-header">
        <h2><i class="bi bi-journal-text"></i> Browse Assignments</h2>
        <p class="text-muted">Find and download assignments for your selected subject</p>
      </div>
      ...
      <?php if ($selectedSemester && $selectedSubject && count($assignments) > 0): ?>
        <div class="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-4">
          <?php foreach ($assignments as $a): 
            // Fetch reviews for this assignment
            ...
            if ($isLoggedIn) {
                ...
                $userReviewStmt->bind_param("ii", $a['id'], $userId);
              ...
              <div class="assignment-card">
                <div class="d-flex justify-content-between align-items-start mb-3">
                  <h5 class="assignment-title mb-0">
                    <i class="bi bi-file-earmark-text me-2"></i><?= htmlspecialchars($a['title']) ?>
                  </h5>
                  <span class="badge bg-primary">Assignment</span>
                </div>
                
                <p class="mb-2"><i class="bi bi-book stats-icon"></i> <?= htmlspecialchars($a['subject_name']) ?></p>
                <p class="mb-2"><i class="bi bi-calendar stats-icon"></i> <?= date('d M Y, h:i A', strtotime($a['uploaded_at'])) ?></p>
                
                <div class="d-flex justify-content-between mb-3">
                  <div>
                    <i class="bi bi-download stats-icon"></i> 
                    <span><?= $a['download_count'] ?> downloads</span>
                    <?php if ($a['user_downloaded'] > 0): ?>
                      <span class="badge bg-success ms-2">Downloaded</span>
                    <?php endif; ?>
                  </div>
                  <?php if ($a['avg_rating']): ?>
                    <div class="text-warning">
                      <?= generateStars($a['avg_rating']) ?>
                      <small class="text-muted">(<?= $a['review_count'] ?>)</small>
                    </div>
                  <?php else: ?>
                ...
                <a href="?semester=<?= $selectedSemester ?>&subject=<?= $selectedSubject ?>&download=<?= $a['id'] ?>" class="btn download-btn w-100 mb-3">
                ...
                <?php if ($isLoggedIn && $a['user_downloaded'] > 0 && !$userReview): ?>
                  <div class="review-form">
                    <h6 class="fw-semibold mb-2"><i class="bi bi-pencil-square me-2"></i>Write a Review</h6>
                    <form method="POST">
                      <input type="hidden" name="material_id" value="<?= $a['id'] ?>">
                      ...
                      <div class="mb-3">
                        <label for="review_text_<?= $a['id'] ?>" class="form-label">Your Review</label>
                        <textarea class="form-control" id="review_text_<?= $a['id'] ?>" name="review_text" rows="2" required></textarea>
                      </div>
                      
                      <button type="submit" name="submit_review" class="btn btn-primary btn-sm w-100">
                ...
                <?php elseif ($userReview): ?>
                  <div class="alert alert-info p-2 mt-2 text-center">
                    <i class="bi bi-check-circle-fill me-1"></i> You've already reviewed this assignment
                  </div>
                ...
                <a href="assignment_reviews.php?material_id=<?= $a['id'] ?>" class="btn btn-sm btn-outline-primary w-100 mt-2">
      ...
      <?php elseif ($selectedSemester && $selectedSubject): ?>
        <div class="alert alert-info text-center py-4">
          ...
          <h5 class="mt-2">No assignments found</h5>
          <p class="mb-0">There are no assignments available for the selected subject.</p>
        </div>
      <?php elseif ($selectedSemester || $selectedSubject): ?>
        <div class="alert alert-warning text-center py-4">
          <i class="bi bi-exclamation-triangle-fill fs-4"></i>
          <h5 class="mt-2">Please select both semester and subject</h5>
          <p class="mb-0">You need to select both semester and subject to view assignments.</p>
        </div>
      <?php else: ?>
        <div class="alert alert-secondary text-center py-4">
          <i class="bi bi-journal-text fs-4"></i>
          <h5 class="mt-2">Get started with assignments</h5>
          <p class="mb-0">Select your semester and subject to browse available assignments.</p>
        </div>
      <?php endif; ?>
    </div>
  </div>
